...
class Renderer:
    def __init__(self,resolution=(600,600),fps=60):
    ...
    def gen_flakes(self):
        ...
        for i in range(self.n_flakes):
            ...
            tmp_vec = [randint(-1,1),randint(-1,1)]
            flakes.append(Flake(i,tmp_pos,radius=randint(2,4),vec=tmp_vec))
    ...
    def draw_flakes(self):
        ...
        for f in self.flakes:
            pygame.draw.ellipse(self.screen,(0,0,0),[f.pos[0],f.pos[1],f.radius,f.radius])
    ...
    def physics(self):
        ...
        for f in self.flakes:
            ...
            tmp_vec = [self.vf.u[self.screen_size[0] - int(f.pos[0])-1][self.screen_size[1] - int(f.pos[1])-1],
...
class VectorField:
    ...
    def gen_field(self,v_type = 'default'):
        ...
        if v_type == 'default':
            u = -np.sin(x**2)
            v = [[1 for i in range(self.resolution[1])] for j in range(self.resolution[0])]
            return x,y,u,v
        if v_type == 'wind':
            u = -x
...
class Flake:
    def __init__(self,f_id,pos,radius=1,vec=[5,1]):
        self.id = f_id
        self.pos = pos
        self.radius = radius
        self.vec = vec
